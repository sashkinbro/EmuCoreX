...
static void vu1FinishThreadedExecution(bool add_cycles)
{
...
static void vu1FinishLocalExecution(bool add_cycles)
{
	const u64 vu1cycles = VU1.cycle;
...
void vu1Finish(bool add_cycles) {
	if (THREAD_VU1)
	{
		vu1FinishThreadedExecution(add_cycles);
		return;
	}

	vu1FinishLocalExecution(add_cycles);
}

static void vu1ExecMicroThreaded(u32 addr)
{
...
static void vu1ExecMicroLocal(u32 addr)
{
	static int count = 0;
