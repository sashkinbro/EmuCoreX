...
std::shared_ptr<InputBinding> InputManager::AddBinding(const std::string_view binding, const InputEventHandler& handler)
{
	...
	for (const std::string_view& chord_binding : chord_bindings)
	{
		...
		{
#ifdef __ANDROID__
			// Android builds don't expose the desktop keyboard hotkey set by default,
			// so skipping the warning keeps startup logs readable.
			if (!binding.starts_with("Keyboard/"))
