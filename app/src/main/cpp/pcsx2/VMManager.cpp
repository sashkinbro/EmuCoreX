...
void VMManager::LoadCoreSettings(SettingsInterface& si)
{
	...
	// Official PCSX2 GameDB fixes are mandatory in EmuCoreX.
...
void VMManager::ApplyGameFixes()
{
	...
	EmuConfig.EnableGameFixes = true;
	game->applyGameFixes(EmuConfig, true);
...
{
	// Local ARM64 policy intentionally keeps the fork's EE/IOP/VU recompiler path alive.
	// Do not "align" this to upstream interpreter fallback unless ARM64 JIT coverage is
	// being replaced with an equally working path.
	recCpu.Reserve();
	psxRec.Reserve();
...
void VMManager::UpdateCPUImplementations()
{
	...
	// This fork intentionally allows ARM64 to keep using the recompiler-backed providers.
	// Upstream ARM64 currently prefers interpreter providers here, but that is not a safe
...
void VMManager::CheckForCPUConfigChanges(const Pcsx2Config& old_config)
{
	...
	// new config, then update conditional handlers/fastmem state, and only then mark
	// provider swaps for deferred execution if the active CPU implementation changed.
	// This path is runtime-sensitive for ARM64 JIT stability even though the logic is
	// currently upstream-aligned.
	Internal::ClearCPUExecutionCaches();
	memBindConditionalHandlers();
