...
static std::string s_discord_display_name = "EmuCoreX";
...
JNIEnv* AndroidDiscordGetEnv()
{
	...
	jclass AndroidDiscordGetBridgeClass(JNIEnv* env)
	{
		...
		if (!s_bridge_class)
		{
			jclass local = env->FindClass("com/sbro/emucorex/core/DiscordBridge");
			if (!local)
				return nullptr;
	...
	void AndroidDiscordApplyPresenceLocked()
	{
		...
		activity.SetName(s_discord_display_name.empty() ? std::string("EmuCoreX") : s_discord_display_name);
		...
		if (has_large_asset)
		{
			...
			assets.SetLargeText(s_discord_display_name.empty() ? std::string("EmuCoreX") : s_discord_display_name);
