...
namespace Achievements
{
	...
	/// Returns the normalized ELF name used by the RetroAchievements hash algorithm.
	std::string_view GetELFNameForHash(const std::string& elf_path);

	/// Computes the RetroAchievements hash for a PS2 ELF path.
	...
	/// Returns the logged-in user's score.
	u32 GetLoggedInUserScore();

	/// Returns the logged-in user's softcore score.
	u32 GetLoggedInUserSoftcoreScore();

	/// Returns the logged-in user's unread message count.
	...
	/// Returns the current game's total achievement count.
	u32 GetGameAchievementCount();

	/// Returns the current game's unlocked achievement count.
	u32 GetUnlockedGameAchievementCount();

	/// Returns the current game's total points.
	u32 GetGamePoints();

	/// Returns the current game's unlocked points.
	u32 GetUnlockedGamePoints();
