...
#ifdef __ANDROID__
	if (!s_output_stream && EmuConfig.SPU2.Backend != AudioBackend::Oboe)
	{
		const std::string primary_error = error.GetDescription();
		Console.Warning("Primary audio backend failed on Android: %s", primary_error.c_str());
		Console.WriteLn("Retrying audio stream creation with Oboe backend...");

		Error oboe_error;
		s_output_stream = AudioStream::CreateStream(AudioBackend::Oboe, sample_rate, EmuConfig.SPU2.StreamParameters,
			"", "", EmuConfig.SPU2.IsTimeStretchEnabled(), &oboe_error);
		if (!s_output_stream)
		{
			Error::SetStringFmt(&error, "Primary backend failed: {}\nOboe fallback failed: {}", primary_error, oboe_error.GetDescription());
		}
		else
		{
			Console.WriteLn("Oboe audio fallback initialized successfully.");
		}
	}
#endif
	if (!s_output_stream)
