		...
		namespace OpcodeImpl
		{
			...
			void CACHE()
			{
				...
				switch (_Rt_)
				{
					...
					{
						// We do not model the EE I-cache contents directly, but games can
						// rely on IXIN after patching exception vectors/handlers in RAM.
						// Invalidate the corresponding recompiled guest code line so the
						// next execution observes freshly written instructions.
						Cpu->Clear(addr & ~0x3Fu, 0x10);
						break;
					}
