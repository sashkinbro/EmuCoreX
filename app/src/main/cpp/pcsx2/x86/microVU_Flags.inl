...
__fi void mVUdivSet(mV)
{
	...
	{
        const x32& reg32 = getFlagReg(sFLAG.write);
		if (!sFLAG.doFlag) {
//            xMOV(getFlagReg(sFLAG.write), getFlagReg(sFLAG.lastWrite));
            armAsm->Mov(a64::WRegister(reg32), a64::WRegister(getFlagReg(sFLAG.lastWrite)));
        }
//		xAND(getFlagReg(sFLAG.write), 0xfff3ffff);
        armAsm->And(a64::WRegister(reg32), a64::WRegister(reg32), 0xfff3ffff);
//		xOR(getFlagReg(sFLAG.write), ptr32[&mVU.divFlag]);
        armAsm->Ldr(a64::WRegister(gprT1), PTR_MVU(microVU[mVU.index].divFlag));
        armAsm->Orr(a64::WRegister(reg32), a64::WRegister(reg32), a64::WRegister(gprT1));
	}
}
...
//			xMOV(gprF0, getFlagReg(bStatus[0]));
            armAsm->Mov(a64::WRegister(gprF0), a64::WRegister(getFlagReg(bStatus[0])));
//			xMOV(gprF1, getFlagReg(bStatus[1]));
            armAsm->Mov(a64::WRegister(gprF1), a64::WRegister(getFlagReg(bStatus[1])));
//			xMOV(gprF2, getFlagReg(bStatus[2]));
            armAsm->Mov(a64::WRegister(gprF2), a64::WRegister(getFlagReg(bStatus[2])));
//			xMOV(gprF3, getFlagReg(bStatus[3]));
            armAsm->Mov(a64::WRegister(gprF3), a64::WRegister(getFlagReg(bStatus[3])));
        }
        else if (sortRegs == 2)
        {
//			xMOV(gprT1, getFlagReg (bStatus[3]));
            armAsm->Mov(a64::WRegister(gprT1), a64::WRegister(getFlagReg(bStatus[3])));
//			xMOV(gprF0, getFlagReg (bStatus[0]));
            armAsm->Mov(a64::WRegister(gprF0), a64::WRegister(getFlagReg(bStatus[0])));
//			xMOV(gprF1, getFlagReg2(bStatus[1]));
            armAsm->Mov(a64::WRegister(gprF1), a64::WRegister(getFlagReg2(bStatus[1])));
//			xMOV(gprF2, getFlagReg2(bStatus[2]));
            armAsm->Mov(a64::WRegister(gprF2), a64::WRegister(getFlagReg2(bStatus[2])));
//			xMOV(gprF3, gprT1);
            armAsm->Mov(a64::WRegister(gprF3), a64::WRegister(gprT1));
        }
        else if (sortRegs == 3)
        {
            int gFlag = (bStatus[0] == bStatus[1]) ? bStatus[2] : bStatus[1];
//			xMOV(gprT1, getFlagReg (gFlag));
            armAsm->Mov(a64::WRegister(gprT1), a64::WRegister(getFlagReg(gFlag)));
//			xMOV(gprT2, getFlagReg (bStatus[3]));
            armAsm->Mov(a64::WRegister(gprT2), a64::WRegister(getFlagReg(bStatus[3])));
//			xMOV(gprF0, getFlagReg (bStatus[0]));
            armAsm->Mov(a64::WRegister(gprF0), a64::WRegister(getFlagReg(bStatus[0])));
//			xMOV(gprF1, getFlagReg3(bStatus[1]));
            armAsm->Mov(a64::WRegister(gprF1), a64::WRegister(getFlagReg3(bStatus[1])));
//			xMOV(gprF2, getFlagReg4(bStatus[2]));
            armAsm->Mov(a64::WRegister(gprF2), a64::WRegister(getFlagReg4(bStatus[2])));
//			xMOV(gprF3, gprT2);
            armAsm->Mov(a64::WRegister(gprF3), a64::WRegister(gprT2));
        }
        else
        {
...
//			xMOV(gprT1, getFlagReg(bStatus[0]));
            armAsm->Mov(a64::WRegister(gprT1), a64::WRegister(getFlagReg(bStatus[0])));
//			xMOV(gprT2, getFlagReg(bStatus[1]));
            armAsm->Mov(a64::WRegister(gprT2), a64::WRegister(getFlagReg(bStatus[1])));
//			xMOV(temp3, getFlagReg(bStatus[2]));
            armAsm->Mov(a64::WRegister(temp3), a64::WRegister(getFlagReg(bStatus[2])));
//			xMOV(gprF3, getFlagReg(bStatus[3]));
            armAsm->Mov(a64::WRegister(gprF3), a64::WRegister(getFlagReg(bStatus[3])));
//			xMOV(gprF0, gprT1);
            armAsm->Mov(a64::WRegister(gprF0), a64::WRegister(gprT1));
//			xMOV(gprF1, gprT2);
            armAsm->Mov(a64::WRegister(gprF1), a64::WRegister(gprT2));
//			xMOV(gprF2, temp3);
            armAsm->Mov(a64::WRegister(gprF2), a64::WRegister(temp3));
