...
//		xMOV(gprF0, ptr32[&mVU.regs().micro_statusflags[0]]);
        armAsm->Ldr(a64::WRegister(gprF0), PTR_CPU(vuRegs[mVU.index].micro_statusflags[0]));
//		xMOV(gprF1, ptr32[&mVU.regs().micro_statusflags[1]]);
        armAsm->Ldr(a64::WRegister(gprF1), PTR_CPU(vuRegs[mVU.index].micro_statusflags[1]));
//		xMOV(gprF2, ptr32[&mVU.regs().micro_statusflags[2]]);
        armAsm->Ldr(a64::WRegister(gprF2), PTR_CPU(vuRegs[mVU.index].micro_statusflags[2]));
//		xMOV(gprF3, ptr32[&mVU.regs().micro_statusflags[3]]);
        armAsm->Ldr(a64::WRegister(gprF3), PTR_CPU(vuRegs[mVU.index].micro_statusflags[3]));
...
static void mVUGenerateWaitMTVU(mV)
{
    ...
    for (i = 0; i < static_cast<int>(iREGCNT_GPR); ++i)
    {
        ...
        if (i == static_cast<int>(gprT2.GetCode()))
            continue;
    ...
    for (i = 0; i < static_cast<int>(iREGCNT_XMM); ++i)
    {
        ...
        armAsm->Push(a64::q31, a64::QRegister(i));
    }
...
//	xFastCall((void*)mVUwaitMTVU);
    ...
    for (i = static_cast<int>(iREGCNT_XMM - 1); i >= 0; --i)
    {
        ...
        armAsm->Pop(a64::QRegister(i), a64::q31);
    }
    //
