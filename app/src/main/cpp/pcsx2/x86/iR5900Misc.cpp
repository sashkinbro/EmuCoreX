...
{
	EE::Profiler.EmitOp(eeOpcode::CACHE);

	// CACHE is rare enough that interpreter fallback is fine, but it still has
	// real behavior for a few BIOS/game paths. If it redirects control flow
	// through an exception handler, bail out of the current block immediately.
	recCall(R5900::Interpreter::OpcodeImpl::CACHE);

	a64::Label cache_continue;
	armLoad(EAX, PTR_CPU(cpuRegs.pc));
	armAsm->Cmp(EAX, pc);
	armAsm->B(&cache_continue, a64::eq);
	armAsm->Ret();
	armBind(&cache_continue);
}
