...
//			xMOV(gprF0, ptr32[&vuRegs->VI[REG_STATUS_FLAG].UL]);
            armAsm->Ldr(a64::WRegister(gprF0), PTR_CPU(vuRegs[0].VI[REG_STATUS_FLAG].UL));
		}
	}
}
...
//			xMOV(ptr32[&vuRegs->VI[REG_STATUS_FLAG].UL], gprF0);
            armAsm->Str(a64::WRegister(gprF0), PTR_CPU(vuRegs[0].VI[REG_STATUS_FLAG].UL));
		}
	}
...
bool mVUIsReservedCOP2(int hostreg)
{
	...
	return (hostreg == static_cast<int>(gprT1.GetCode()) ||
		hostreg == static_cast<int>(gprT2.GetCode()) ||
		hostreg == static_cast<int>(gprF0.GetCode()));
}
...
#define INTERPRETATE_COP2_FUNC(f) \
	...
	{ \
		...
		armAdd64(PTR_CPU(cpuRegs.cycle), scaleblockcycles_clear()); \
		recCall(V##f); \
	}
...
//	xTEST(ptr32[&VU0.VI[REG_VPU_STAT].UL], 0x100);
    armAsm->Tst(armLoad(PTR_CPU(vuRegs[0].VI[REG_VPU_STAT].UL)), 0x100);
...
//			xMOV(ptr32[&cpuRegs.cycle], eax); // update cycles
            armAdd64(RAX, PTR_CPU(cpuRegs.cycle), scaleblockcycles_clear());

//			xTEST(ptr32[&VU0.VI[REG_VPU_STAT].UL], 0x1);
            armAsm->Tst(armLoad(PTR_CPU(vuRegs[0].VI[REG_VPU_STAT].UL)), 0x1);
...
//				xSUB(eax, ptr32[&VU0.cycle]);
                armAsm->Sub(RAX, RAX, armLoad64(PTR_CPU(vuRegs[0].cycle)));
...
//                    xSUB(eax, ptr32[&VU0.nextBlockCycles]);
                    armAsm->Sub(RAX, RAX, armLoad64(PTR_CPU(vuRegs[0].nextBlockCycles)));
                }
//				xCMP(eax, 4);
                armAsm->Cmp(RAX, 4);
//				xForwardJL32 skip;
...
//			xMOV(ptr32[&vu0Regs.VI[REG_R].UL], eax);
            armAsm->Str(a64::WRegister(EAX), PTR_CPU(vuRegs[0].VI[REG_R].UL));
			break;
...
//					xMOV(ptr32[&vu0Regs.VI[REG_FBRST].UL], 0);
                    armStore(PTR_CPU(vuRegs[0].VI[REG_FBRST].UL), 0);
					return;
				}

#if defined(_M_ARM64) || defined(__aarch64__)
				_eeMoveGPRtoR(EAX, _Rt_);
				armStore(PTR_CPU(vuRegs[0].VI[REG_FBRST].UL), EAX);

				iFlushCall(FLUSH_FREE_VU0);

				armAsm->Tst(armLoad(PTR_CPU(vuRegs[0].VI[REG_FBRST].UL)), 0x002);
				{
					a64::Label skip_vu0_reset;
					armAsm->B(&skip_vu0_reset, a64::Condition::eq);
					armEmitCall(reinterpret_cast<void*>(vu0ResetRegs));
					armBind(&skip_vu0_reset);
				}

				armAsm->Tst(armLoad(PTR_CPU(vuRegs[0].VI[REG_FBRST].UL)), 0x200);
				{
					a64::Label skip_vu1_reset;
					armAsm->B(&skip_vu1_reset, a64::Condition::eq);
					armEmitCall(reinterpret_cast<void*>(vu1ResetRegs));
					armBind(&skip_vu1_reset);
				}

				armAsm->And(EAX, armLoad(PTR_CPU(vuRegs[0].VI[REG_FBRST].UL)), 0x0C0C);
				armAsm->Str(a64::WRegister(EAX), PTR_CPU(vuRegs[0].VI[REG_FBRST].UL));
				break;
#endif
...
static void recQMTC2()
{
	...
	if (_Rt_)
	{
        ...
        if (can_rename && rtreg >= 0 && rtreg != static_cast<int>(xmmPQ.GetCode()))
		{
