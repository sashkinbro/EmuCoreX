...
//	xOR(eax, ptr[(&psHu32(DMAC_STAT))]);
    armLoadPtr(EEX, &psHu32(DMAC_STAT));
    armAsm->Orr(EAX, EAX, EEX);
//	xAND(eax, ecx);
...
//		xMOV(ptr64[&cpuRegs.cycle], rcx); // update cycles
        armAdd64(RCX, PTR_CPU(cpuRegs.cycle), scaleblockcycles_clear());
//		xMOV(rax, rcx);
        armAsm->Mov(RAX, RCX);
//		xSUB(rax, ptr[&cpuRegs.lastCOP0Cycle]);
        armAsm->Sub(RAX, RAX, armLoad64(PTR_CPU(cpuRegs.lastCOP0Cycle)));
//		xADD(ptr[&cpuRegs.CP0.n.Count], eax);
        armAdd(PTR_CPU(cpuRegs.CP0.n.Count), EAX);
//		xMOV(ptr[&cpuRegs.lastCOP0Cycle], rcx);
        armAsm->Str(RCX, PTR_CPU(cpuRegs.lastCOP0Cycle));
...
//					xMOV(ptr[&cpuRegs.lastPERFCycle[0]], rax);
                    armAsm->Str(RAX, PTR_CPU(cpuRegs.lastPERFCycle[0]));
				}
				else // MTPC 1
				{
//					xMOV(rax, ptr64[&cpuRegs.cycle]);
...
//					xMOV(ptr[&cpuRegs.lastPERFCycle[1]], rax);
                    armAsm->Str(RAX, PTR_CPU(cpuRegs.lastPERFCycle[1]));
				}
				break;
...
//					xMOV(ptr[&cpuRegs.lastPERFCycle[0]], rcx);
                    armAsm->Str(RCX, PTR_CPU(cpuRegs.lastPERFCycle[0]));
				}
				else // MTPC 1
				{
//					xMOV(rcx, ptr64[&cpuRegs.cycle]);
...
//					xMOV(ptr[&cpuRegs.lastPERFCycle[1]], rcx);
                    armAsm->Str(RCX, PTR_CPU(cpuRegs.lastPERFCycle[1]));
				}
				break;
