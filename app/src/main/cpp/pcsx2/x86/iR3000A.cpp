...
void psxSetBranchReg(u32 reg)
{
	...
	if (reg != 0xffffffff)
	{
		...
		if (!swap)
		{
			// Preserve the JR/JALR target across delay-slot recompilation without
			// consuming another callee-saved host register on ARM64.
			_psxMoveGPRtoM(PTR_CPU(psxRegs.pcWriteback), reg);
...
//		xMOV(rax, ptr64[&psxRegs.cycle]);
        armAsm->Ldr(RAX, PTR_CPU(psxRegs.cycle));
//		xMOV(rcx, rax);
        armAsm->Mov(RCX, RAX);
//		xMOV(edx, ptr32[&psxRegs.iopCycleEE]);
        armLoad(EDX, PTR_CPU(psxRegs.iopCycleEE));
//		xADD(edx, 7);
        armAsm->Add(EDX, EDX, 7);
//		xSHR(edx, 3);
        armAsm->Lsr(EDX, EDX, 3);
//		xADD(rax, rdx);
        armAsm->Add(RAX, RAX, RDX);
//		xCMP(rax, ptr64[&psxRegs.iopNextEventCycle]);
        armAsm->Ldr(REX, PTR_CPU(psxRegs.iopNextEventCycle));
        armAsm->Cmp(RAX, REX);
//		xCMOVNS(rax, ptr64[&psxRegs.iopNextEventCycle]);
        armAsm->Csel(RAX, REX, RAX, a64::Condition::pl);
//		xMOV(ptr64[&psxRegs.cycle], rax);
        armAsm->Str(RAX, PTR_CPU(psxRegs.cycle));
//		xSUB(rax, rcx);
        armAsm->Sub(RAX, RAX, RCX);
//		xSHL(rax, 3);
        armAsm->Lsl(RAX, RAX, 3);
...
//			xCMP(ptr32[&psxRegs.pc], newpc);
            armLoad(EEX, PTR_CPU(psxRegs.pc));
            armAsm->Cmp(EEX, newpc);
...
//		xMOV(ptr64[&psxRegs.cycle], rbx); // update cycles
        armAdd64(RBX, PTR_CPU(psxRegs.cycle), blockCycles);
...
//		xSUB(rbx, ptr64[&psxRegs.iopNextEventCycle]);
        armAsm->Subs(RBX, RBX, armLoad64(PTR_CPU(psxRegs.iopNextEventCycle)));
...
//	xADD(ptr64[&psxRegs.cycle], psxScaleBlockCycles());
    armAdd64(PTR_CPU(psxRegs.cycle), psxScaleBlockCycles());
