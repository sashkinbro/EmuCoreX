...
#define SHIFT_XYZW(gprReg) \
	do { \
		...
		{ \
			armAsm->Lsl(a64::WRegister(gprReg), a64::WRegister(gprReg), ADD_XYZW); \
		} \
	} while (0)
...
//            xAND(sReg, 0xfffc00ff); // Clear O,U,S,Z flags
            armAsm->And(a64::WRegister(sReg), a64::WRegister(sReg), 0xfffc00ff);
        }
	}
...
//	xAND(mReg, AND_XYZW); // Grab "Is Signed" bits from the previous calculation
    armAsm->And(a64::WRegister(mReg), a64::WRegister(mReg), AND_XYZW);
//	xSHL(mReg, 4);
    armAsm->Lsl(a64::WRegister(mReg), a64::WRegister(mReg), 4);

	//-------------------------Check for Zero flags------------------------------

//	xAND(gprT2, AND_XYZW); // Grab "Is Zero" bits from the previous calculation
    armAsm->And(a64::WRegister(gprT2), a64::WRegister(gprT2), AND_XYZW);
//	xOR(mReg, gprT2);
    armAsm->Orr(a64::WRegister(mReg), a64::WRegister(mReg), a64::WRegister(gprT2));
...
//		xForwardJump32 oJMP(Jcc_Zero);
        a64::Label oJMP;
        armAsm->Cbz(a64::WRegister(gprT2), &oJMP);

//		xOR(sReg, 0x820000);
        armAsm->Orr(a64::WRegister(sReg), a64::WRegister(sReg), 0x820000);
		if (mFLAG.doFlag)
		{
//			xSHL(gprT2, 12); // Add the results to the MAC Flag
            armAsm->Lsl(a64::WRegister(gprT2), a64::WRegister(gprT2), 12);
//			xOR(mReg, gprT2);
            armAsm->Orr(a64::WRegister(mReg), a64::WRegister(mReg), a64::WRegister(gprT2));
		}
...
//		xAND(mReg, 0xFF); // Ignore overflow bits, they're handled separately
        armAsm->And(a64::WRegister(mReg), a64::WRegister(mReg), 0xFF);
//		xOR(sReg, mReg);
        armAsm->Orr(a64::WRegister(sReg), a64::WRegister(sReg), a64::WRegister(mReg));
		if (sFLAG.doNonSticky)
		{
//			xSHL(mReg, 8);
            armAsm->Lsl(a64::WRegister(mReg), a64::WRegister(mReg), 8);
//			xOR(sReg, mReg);
            armAsm->Orr(a64::WRegister(sReg), a64::WRegister(sReg), a64::WRegister(mReg));
		}
	}
