...
static void rpsxJALR()
{
	...
	if (!swap)
	{
		// Preserve the pre-delay-slot JALR target in memory so the emitted delay-slot
		// path can't clobber it before we commit the final PC.
		_psxMoveGPRtoM(PTR_CPU(psxRegs.pcWriteback), _Rs_);
	}
