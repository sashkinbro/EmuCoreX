...
//                    mVUsaveReg(xmm(i), ptr[&getVF(mapX.VFreg)], mapX.xyzw, 1);
	...
	{
		if (regId != static_cast<int>(xmmPQ.GetCode()) && ((xmmMap[regId].VFreg == 33 && !EmuConfig.Gamefixes.IbitHack) || xmmMap[regId].isZero))
			return false;
		else
			return true;
	}
...
//                xMOV(ptr32[&regs().VI[mapX.VIreg].UL], xRegister32(reg));
                armAsm->Str(a64::WRegister(reg), PTR_CPU(vuRegs[index].VI[mapX.VIreg].UL));
            }
			if (clearDirty)
			{
				mapX.dirty = false;
...
//									xMOV(gprX, ptr32[&regs().VI[viWriteReg].UL]);
									armAsm->Ldr(gprX, PTR_CPU(vuRegs[index].VI[viWriteReg].UL));
								}
								writeVIBackup(gprX);
								backup = false;
							}
...
//				xMOV(gprX, ptr32[&regs().VI[viLoadReg].UL]);
				armAsm->Ldr(gprX, PTR_CPU(vuRegs[index].VI[viLoadReg].UL));
			}
        }
        else if (viLoadReg == 0) {
//            xXOR(gprX, gprX);
