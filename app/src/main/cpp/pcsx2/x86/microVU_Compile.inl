...
//			xMOV(ptr32[&mVU.getVI(REG_I)], tempI);
            armStore(PTR_CPU(vuRegs[mVU.index].VI[REG_I]), tempI);
		}
		incPC(1);
	}
}
...
//        xMOV(ptr32[&mVU.regs().nextBlockCycles], mVUcycles);
        armStore(PTR_CPU(vuRegs[mVU.index].nextBlockCycles), mVUcycles);
    }
...
//                    xMOV(ptr32[&mVU.regs().nextBlockCycles], 0);
                    armStore(PTR_CPU(vuRegs[mVU.index].nextBlockCycles), 0);
                }
				mVUendProgram(mVU, &mFC, 0);
