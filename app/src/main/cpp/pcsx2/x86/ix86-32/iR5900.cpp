...
// Shared emitter internals are also needed by the ARM64 dynarec port.
...
//	xMOV(rax, ptr64[&cpuRegs.cycle]);
    armAsm->Ldr(RAX, PTR_CPU(cpuRegs.cycle));
//	xMOV(ptr64[&cpuRegs.nextEventCycle], rax);
    armAsm->Str(RAX, PTR_CPU(cpuRegs.nextEventCycle));
...
//		xMOV(rax, ptr64[&cpuRegs.nextEventCycle]);
        armAsm->Ldr(RAX, PTR_CPU(cpuRegs.nextEventCycle));
//		xADD(ptr64[&cpuRegs.cycle], scaleblockcycles());
        armAdd64(PTR_CPU(cpuRegs.cycle), scaleblockcycles());
//		xCMP(rax, ptr64[&cpuRegs.cycle]);
        armAsm->Ldr(REX, PTR_CPU(cpuRegs.cycle));
        armAsm->Cmp(RAX, REX);
//		xCMOVS(rax, ptr64[&cpuRegs.cycle]);
        armAsm->Csel(RAX, REX, RAX, a64::Condition::mi);
...
//		xMOV(ptr64[&cpuRegs.cycle], rax); // update cycles
        armAdd64(RAX, PTR_CPU(cpuRegs.cycle), scaleblockcycles());
//		xSUB(rax, ptr64[&cpuRegs.nextEventCycle]);
        armAsm->Subs(RAX, RAX, armLoad64(PTR_CPU(cpuRegs.nextEventCycle)));
...
//	xMOV(rbx, ptr64[&cpuRegs.cycle]); // ebx = cycle
    armAsm->Ldr(RBX, PTR_CPU(cpuRegs.cycle));
//	xMOV(rcx, ptr64[&cpuRegs.nextEventCycle]); // ecx = nextEventCycle
    armAsm->Ldr(RCX, PTR_CPU(cpuRegs.nextEventCycle));
//	xCMP(rbx, rcx);
    armAsm->Cmp(RBX, RCX);
...
//	xSUB(rax, rbx); // new_cycles -= cycle
    armAsm->Sub(RAX, RAX, RBX);
//	xSHR(rax, 3); // compute new v0 value
    armAsm->Lsr(RAX, RAX, 3);
//	xSUB(rdx, rax); // v0 -= cycle_diff
    armAsm->Subs(RDX, RDX, RAX);
