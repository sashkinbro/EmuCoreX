...
// x86 keeps extra padding here so the generated load/store can be backpatched
// into a larger jump sequence. ARM64 patches a single 4-byte `B` instruction in
// `armEmitJmpPtr()`, so no extra padding contract is needed on this path.

//#define LOG_STORES

static bool ShouldTracePotentialTargetDescStore(u32 guest_pc)
{
	(void)guest_pc;
	return false;
}
...
//				_freeXMMreg(xRegisterSSE::GetArgRegister(1, 0).GetId());
                _freeXMMreg(armQRegister(0).GetCode());
...
//				xMOVAPS(xmm0, ptr128[arg1reg]);
                armAsm->Ldr(armQRegister(xmm0.GetId()).Q(), mop);
                break;
...
//				xMOVAPS(ptr[arg1reg], xRegisterSSE::GetArgRegister(1, 0));
                armAsm->Str(armQRegister(0).Q(), mop);
                break;
		}
	}
} // namespace vtlb_private
...
int vtlb_DynGenReadNonQuad_Const(u32 bits, bool sign, bool xmm, u32 addr_const, vtlb_ReadRegAllocCallback dest_reg_alloc)
{
    ...
    if (!vmv.isHandler(addr_const))
    {
        ...
        const auto mop = armMemOperandPtr(reinterpret_cast<const void*>(ppf));
...
//            xMOVAPS(xRegisterSSE(reg), xmm0);
            armAsm->Mov(armQRegister(reg), armQRegister(xmm0.GetId()));
        }
...
//			const xRegisterSSE argreg(xRegisterSSE::GetArgRegister(1, 0));
            const a64::VRegister argreg(armQRegister(0));
	...
	u8 size_in_bits, bool is_signed, bool is_load, bool is_xmm)
{
	...
	const auto should_save_gpr = [&](u32 i) {
		return (i == rbxid || i == arg1id || i == arg2id || i == arg3id ||
			xRegisterBase::IsCallerSaved(i) || armIsCallerSaved(i));
	};
	const auto should_save_xmm = [&](u32 i) {
		return (xRegisterSSE::IsCallerSaved(i) || armIsCallerSavedXmm(i));
	};
	...
	for (u32 i = 0; i < iREGCNT_GPR; ++i)
	{
		if ((gpr_bitmask & (1u << i)) &&
			should_save_gpr(i) &&
			(!is_load || is_xmm || data_register != i))
		{
			num_gprs++;
		}
	}
	...
	{
		if ((fpr_bitmask & (1u << i)) && should_save_xmm(i) && (!is_load || !is_xmm || data_register != i))
			num_fprs++;
	}
...
//                xMOVAPS(xRegisterSSE(data_register), xmm0);
                armAsm->Mov(armQRegister(data_register), armQRegister(xmm0.GetId()));
            }
		}
		else
		{
...
//				xMOVDZX(xRegisterSSE(data_register), rax);
                armAsm->Fmov(a64::QRegister(data_register).S(), EAX);
			}
			else
			{
...
//            xMOV(arg1regd, xRegister32(address_register));
		...
		{
//			const xRegisterSSE argreg(xRegisterSSE::GetArgRegister(1, 0));
            const a64::QRegister argreg(armQRegister(0));
...
//				xMOVD(arg2reg, xRegisterSSE(data_register));
                armAsm->Fmov(EDX, a64::QRegister(data_register).S());
			}
			else
			{
