...
static void recMFHILO(bool hi, bool upper)
{
	...
	if (upper)
	{
		...
		// Keep the cached lower 64-bits of rd/hi/lo coherent by flushing any wide/XMM state
		// before we directly update only the upper HI/LO lane and the low 64-bits of rd.
		GPR_DEL_CONST(_Rd_);
		_deleteGPRtoXMMreg(_Rd_, DELETE_REG_FLUSH_AND_FREE);
		_deleteGPRtoX86reg(_Rd_, DELETE_REG_FREE_NO_WRITEBACK);
		_deleteGPRtoXMMreg(reg, DELETE_REG_FLUSH_AND_FREE);
		_deleteGPRtoX86reg(reg, DELETE_REG_FLUSH_AND_FREE);

		const a64::MemOperand hilo_ptr = hi ? PTR_CPU(cpuRegs.HI.UD[1]) : PTR_CPU(cpuRegs.LO.UD[1]);
		const int gprd = _allocIfUsedGPRtoX86(_Rd_, MODE_WRITE);
		if (gprd >= 0)
		{
			armLoad(a64::XRegister(gprd), hilo_ptr);
		}
		else
		{
			armLoad(RAX, hilo_ptr);
...
//                xMOVHL.PS(xRegisterSSE(xmmd), xRegisterSSE(xmmhilo));
                armAsm->Mov(a64::QRegister(xmmd).V2D(), 0, a64::QRegister(xmmhilo).V2D(), 1);
            }
			else {
...
static void recMTHILO(bool hi, bool upper)
{
	...
	if (upper)
	{
		// Preserve the lower 64-bits already held in HI/LO, then invalidate cached wide state
		// before writing only the upper 64-bits from rs.
		_deleteGPRtoXMMreg(reg, DELETE_REG_FLUSH_AND_FREE);
		_deleteGPRtoX86reg(reg, DELETE_REG_FLUSH_AND_FREE);

		const a64::MemOperand hilo_ptr = hi ? PTR_CPU(cpuRegs.HI.UD[1]) : PTR_CPU(cpuRegs.LO.UD[1]);
		const int gprs = _allocIfUsedGPRtoX86(_Rs_, MODE_READ);
		if (gprs >= 0)
		{
			armStore(hilo_ptr, a64::XRegister(gprs));
		}
		else
		{
			_eeMoveGPRtoR(RAX, _Rs_);
			armStore(hilo_ptr, RAX);
		}
		return;
	}
#endif
