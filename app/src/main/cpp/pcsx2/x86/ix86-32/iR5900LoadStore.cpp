...
static __fi u32 GetFaultingGuestPC()
{
...
void recLWL()
{
	...
	// Recompute the byte offset after the read instead of keeping a callee-saved
	// temporary alive across the VTLB path. This avoids ARM64 GPR allocator
	// pressure in recLWL/recLWR().
    _eeMoveGPRtoR(ECX, _Rs_);
	if (_Imm_ != 0) {
        armAsm->Add(ECX, ECX, _Imm_);
    }
    armAsm->And(ECX, ECX, 3);
    armAsm->Lsl(ECX, ECX, 3);
...
void recLWR()
{
    ...
    const a64::WRegister temp(_allocX86reg(X86TYPE_TEMP, 0, 0));
	const int treg = _allocX86reg(X86TYPE_GPR, _Rt_, MODE_READ | MODE_WRITE);
    auto reg32 = a64::WRegister(treg);
    auto reg64 = a64::XRegister(treg);
    ...
    a64::Label nomask;
    armAsm->Cbz(ECX, &nomask);
    armAsm->Lsl(ECX, ECX, 3);
	// mask off bytes loaded
    armAsm->Mov(temp, 24);
    armAsm->Sub(temp, temp, ECX);
    armAsm->Mov(EDX, 0xffffff00);
    armAsm->Lsl(EDX, EDX, temp);
    armAsm->And(EDX, EDX, reg32);

	// OR in bytes loaded
    armAsm->Lsr(EAX, EAX, ECX);
    armAsm->Orr(EDX, EDX, EAX);
    armAsm->Bfi(reg64, RDX, 0, 32);
...
//        xADD(arg1regd, _Imm_);
	...
	// Check the low address bits before the read, then recompute them afterwards.
    armAsm->And(EDX, ECX, 3);
    armAsm->Cmp(EDX, 3);
    armAsm->And(ECX, ECX, ~3);

	// If we're not using fastmem, we need to flush early. Because the first read
	// (which would flush) happens inside a branch.
	if (!CHECK_FASTMEM || vtlb_IsFaultingPC(GetFaultingGuestPC()))
...
//	xForwardJE8 skip;
    ...
    _eeMoveGPRtoR(EDX, _Rs_, false);
    if (_Imm_ != 0)
        armAsm->Add(EDX, EDX, _Imm_);
    armAsm->And(EDX, EDX, 3);
    armAsm->Lsl(EDX, EDX, 3);

	// mask read -> edx
//	xMOV(ecx, 24);
    armAsm->Mov(ECX, 24);
//	xSUB(ecx, temp);
    armAsm->Sub(ECX, ECX, EDX);
...
void recLDL()
{
    ...
    const a64::WRegister temp1(_allocX86reg(X86TYPE_TEMP, 0, 0));
	_freeX86reg(EAX);
	_freeX86reg(ECX);
	_freeX86reg(EDX);
//	_freeX86reg(arg1regd);
...
//		_freeX86reg(arg2regd);
        ...
        const a64::WRegister temp1(_allocX86reg(X86TYPE_TEMP, 0, 0));
        const a64::XRegister temp2(_allocX86reg(X86TYPE_TEMP, 0, 0));
//		_eeMoveGPRtoR(arg2reg, _Rt_);
        _eeMoveGPRtoR(RDX, _Rt_);
