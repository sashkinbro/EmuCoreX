...
namespace Achievements
{
	...
	std::string_view GetELFNameForHash(const std::string& elf_path);
	std::string GetGameHash(const std::string& elf_path);
...
void Achievements::DownloadImage(std::string url, std::string cache_filename)
{
	...
	{
		Console.Error("Refusing to download badge image to an empty cache path.");
		return;
	}

	if (!s_http_downloader)
	{
		Console.Error("Refusing to download badge image '%s' without an active HTTP downloader.", cache_filename.c_str());
		return;
	}
...
bool Achievements::Initialize()
{
	...
	{
		pxAssertRel(s_client && s_http_downloader, "Achievements client/downloader must stay in sync");

		// Android can replay settings changes while the VM thread is still booting.
		// If the client is already alive, treat initialization as idempotent and just
		// refresh the pending game identification for the current VM.
...
bool Achievements::CreateClient(rc_client_t** client, std::unique_ptr<HTTPDownloader>* http)
{
	*http = HTTPDownloader::Create(Host::GetHTTPUserAgent());
	...
	rc_client_set_event_handler(new_client, ClientEventHandler);
	rc_client_set_hardcore_enabled(new_client, s_hardcore_mode);
	rc_client_set_encore_mode_enabled(new_client, EmuConfig.Achievements.EncoreMode);
	rc_client_set_unofficial_enabled(new_client, EmuConfig.Achievements.UnofficialTestMode);
	rc_client_set_spectator_mode_enabled(new_client, EmuConfig.Achievements.SpectatorMode);
...
void Achievements::EnsureCacheDirectoriesExist()
{
	if (EmuFolders::Cache.empty())
	{
		if (!EmuFolders::DataRoot.empty())
			EmuFolders::Cache = Path::Combine(EmuFolders::DataRoot, "cache");
		else if (!EmuFolders::AppRoot.empty())
			EmuFolders::Cache = Path::Combine(EmuFolders::AppRoot, "cache");
	}

	if (EmuFolders::Cache.empty())
	{
		ReportFmtError("Failed to resolve achievements cache root (AppRoot='{}', DataRoot='{}').", EmuFolders::AppRoot, EmuFolders::DataRoot);
		s_image_directory.clear();
		return;
	}
...
bool Achievements::Login(const char* username, const char* password, Error* error)
{
	...
	{
		s_client = client;
		s_http_downloader = std::move(temporary_downloader);
		temporary_client_guard.Cancel();

		// A successful login should leave Android achievements fully enabled so
		// subsequent state refreshes and game boots use the same persistent client.
		if (!EmuConfig.Achievements.Enabled)
		{
		...
		// Keep the persistent client in sync with the configured RA feature flags.
		rc_client_set_hardcore_enabled(s_client, s_hardcore_mode);
		rc_client_set_encore_mode_enabled(s_client, EmuConfig.Achievements.EncoreMode);
		rc_client_set_unofficial_enabled(s_client, EmuConfig.Achievements.UnofficialTestMode);
		rc_client_set_spectator_mode_enabled(s_client, EmuConfig.Achievements.SpectatorMode);
	...
	// If the VM is already running, load the currently active game now.
	if (VMManager::HasValidVM())
		BeginLoadGame();
...
void Achievements::ShowLoginSuccess(const rc_client_t* client)
{
	...
	Host::OnAchievementsLoginSuccess(
		user->display_name ? user->display_name : user->username, user->score, user->score_softcore, user->num_unread_messages);
...
u32 Achievements::GetLoggedInUserScore()
{
	const rc_client_user_t* user = rc_client_get_user_info(s_client);
	return user ? user->score : 0;
}

u32 Achievements::GetLoggedInUserSoftcoreScore()
{
	const rc_client_user_t* user = rc_client_get_user_info(s_client);
...
u32 Achievements::GetGameAchievementCount()
{
	return s_game_summary.num_core_achievements;
}

u32 Achievements::GetUnlockedGameAchievementCount()
{
...
u32 Achievements::GetUnlockedGamePoints()
{
