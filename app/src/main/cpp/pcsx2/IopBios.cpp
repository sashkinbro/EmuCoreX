...
#define Ra3 (iopMemReadString(a3))
	...
	public:
		...
		static int open(IOManFile** file, const std::string& full_path, s32 flags, u16 mode)
		{
			...
			const int hostfd = (file_path.rfind("content://", 0) == 0) ?
				FileSystem::OpenFDFileContent(file_path.c_str()) :
				FileSystem::OpenFDFile(file_path.c_str(), native_flags, native_mode);
			if (hostfd < 0)
