...
fpuRegisters& fpuRegs = g_cpuRegistersPack.fpuRegs;
psxRegisters& psxRegs = g_cpuRegistersPack.psxRegs;
VURegs& VU0 = g_cpuRegistersPack.vuRegs[0];
VURegs& VU1 = g_cpuRegistersPack.vuRegs[1];
cpuRegisters& _cpuRegistersPack = g_cpuRegistersPack.cpuRegs;
...
__ri void cpuException(u32 code, u32 bd)
{
	...
	if (is_tlb_exception && Cpu != &intCpu)
	{
		...
		Cpu->Clear((target_pc & 0x1fffffffu) & ~0x3ffu, 0x100);
	}
