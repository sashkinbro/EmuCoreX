...
void GSRendererHW::Move()
{
	...
	// The CPU fallback consumes the source immediately. It cannot use a frame-old async
	// snapshot, so only this uncommon fallback forces the required source readback.
...
void GSRendererHW::Draw()
{
	...
	if (!no_rt && has_colclip_texture && (m_conf.colclip_frame.FBP != m_cached_ctx.FRAME.FBP ||
		(PRIM->TME && m_conf.colclip_frame.Block() == m_cached_ctx.TEX0.TBP0)))
	{
		GIFRegTEX0 FRAME;
...
void GSRendererHW::HandleFlatShadedVertices()
{
	...
	// If all vertices of each primitive have the same color there is nothing to do.
	bool prims_flat = true;
	for (u32 i = 0; i < m_index.tail; i += n)
	{
		for (u32 j = 0; j < n - 1; j++)
		{
			if (m_vertex.buff[m_index.buff[i + j]].RGBAQ.U32[0] !=
				m_vertex.buff[m_index.buff[i + n - 1]].RGBAQ.U32[0])
			{
				prims_flat = false;
				break;
			}
	...
	GSTextureCache::Target* rt, bool can_scale_rt_alpha, bool& new_rt_alpha_scale)
{
	...
	switch (GSConfig.AccurateBlendingUnit)
	{
		...
		case AccBlendLevel::Medium:
			...
			sw_blending |= barriers_supported && m_vt.m_primclass == GS_SPRITE_CLASS &&
				ComputeDrawlistGetSize(rt->m_scale) < GSMediumBlendSpriteSWLimit(features);
			// We don't want the cases to be enabled if barriers aren't supported so limit it to no overlap.
	...
	const bool force_sw_blending =
		// With framebuffer fetch, prefer software blending when the framebuffer is already needed.
		(features.framebuffer_fetch && (one_barrier || m_conf.require_full_barrier)) ||
		// A depth feedback loop without native depth feedback cannot be mixed with dual-source blending.
		(m_conf.ps.IsFeedbackLoopDepth() &&
	...
	bool& target_region, GSVector2i& unscaled_size, float& scale, GSDevice::RecycledTexture& src_copy)
{
	...
	if (m_downscale_source || m_channel_shuffle || tex->m_texture->IsDepthLike())
	{
		...
		if (m_channel_shuffle && (tex_diff || frame_diff || tex_page_h != frame_page_h || tex_page_v != frame_page_v))
		{
			...
			const GSVector2i draw_offset(
				(static_cast<int>(m_vt.m_min.t.x) / GSLocalMemory::m_psm[m_cached_ctx.TEX0.PSM].pgs.x) * GSLocalMemory::m_psm[m_cached_ctx.FRAME.PSM].pgs.x,
...
void GSRendererHW::EmulateAlphaTest(DATEOptions& date_options)
{
	...
	if (prefer_feedback && !prefer_two_pass && !avoid_feedback)
	{
		...
		m_conf.ps.atst = ps_atst;
		m_conf.cb_ps.FogColor_AREF.a = ps_aref;
		m_conf.ps.afail = static_cast<GSHWDrawConfig::PS_AFAIL>(afail);
...
void GSRendererHW::DetectTextureShuffleSecondPass(GSTextureCache::Target* rt, GSTextureCache::Source* tex)
{
	...
	if (m_texture_shuffle)
	{
		...
		else
		{
			...
			if (m_cached_ctx.TEX0.TBP0 != m_cached_ctx.FRAME.Block() && rt->m_32_bits_fmt &&
				IsOpaque() && !m_vt.IsRealLinear() && HasLowerOnes(m_cached_ctx.FRAME.FBMSK))
			{
	...
	else
	{
		...
		if (PRIM->TME &&
			...
			tex_psm.bpp == 16 && frame_psm.bpp == 16 && rt->m_32_bits_fmt && tex && tex->m_32_bits_fmt)
		{
			GL_INS("HW: Texture shuffle detection (2): Passed (HACK: reinterpreting both source/RT as 16 bit).");
			m_texture_shuffle = true;
...
void GSRendererHW::OffsetDraw(s32 fbp_offset, s32 zbp_offset, s32 xoffset, s32 yoffset)
{
	...
	m_r += GSVector4i(xoffset, yoffset).xyxy();
}
...
void GSRendererHW::StartDepthAsRTFeedback()
{
	...
	if (m_conf.ds && GSUsesDepthAsRTFeedback(g_gs_device->Features(), m_conf.ps.IsFeedbackLoopDepth()))
	{
		...
		if (!m_cached_ds_as_rt || m_cached_ds_as_rt_size.x != w || m_cached_ds_as_rt_size.y != h)
		{
			if (m_cached_ds_as_rt)
				g_gs_device->Recycle(m_cached_ds_as_rt);
			m_cached_ds_as_rt = g_gs_device->CreateRenderTarget(w, h, GSTexture::Format::DepthColor, false, true);
		...
		g_gs_device->StretchRectAuto(m_conf.ds, sRect, m_conf.ds_as_rt, dRect, Nearest);
