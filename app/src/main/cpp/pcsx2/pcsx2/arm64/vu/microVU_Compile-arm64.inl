...
static __fi void mVUCompileLoadCurIToIReg_emit_oaknut(mV);
static __fi void mVUCompileStoreIRegImm_emit_oaknut(mV, u32 imm);
static __fi void mVUCompileBadOpCall_emit_oaknut(mV);
static __fi void mVUCompileSaveLpState_emit_oaknut(mV, const u32* state);
static __fi void mVUCompileMoveQ_emit_oaknut(int dst, int src);
static __fi void mVUCompileXorQ_emit_oaknut(int dst, int lhs, int rhs);
...
{
	recBeginOaknutEmit();
	oakAsm->MOV(oak::util::W0, mVU.prog.cur->idx);
	oakAsm->MOV(oak::util::W1, xPC);
	oakEmitCall(!isVU1 ?
		reinterpret_cast<const void*>(mVUbadOp0) :
		reinterpret_cast<const void*>(mVUbadOp1));
	recEndOaknutEmit();
}
...
{
	recBeginOaknutEmit();
	oakAsm->MOV(oakQRegister(dst).B16(), oakQRegister(src).B16());
	recEndOaknutEmit();
}
...
{
	recBeginOaknutEmit();
	oakAsm->EOR(oakQRegister(dst).B16(), oakQRegister(lhs).B16(), oakQRegister(rhs).B16());
	recEndOaknutEmit();
}
...
void doIbit(mV)
{
	if (mVUup.iBit)
	{
		...
		{
			mVUCompileLoadCurIToIReg_emit_oaknut(mVU);
		}
		else
		{
			u32 tempI;
			...
			mVUCompileStoreIRegImm_emit_oaknut(mVU, tempI);
		}
		incPC(1);
	}
}
...
void doSwapOp(mV)
{
	if (mVUinfo.backupVF && !mVUlow.noWriteVF)
	{
		...
		const int t1 = mVU.regAlloc->allocRegId(mVUlow.VF_write.reg);
		const int t2 = mVU.regAlloc->allocRegId();
		mVUCompileMoveQ_emit_oaknut(t2, t1);
		mVU.regAlloc->clearNeededXmmId(t1);

		mVUopL(mVU, 1);

		const int t3 = mVU.regAlloc->allocRegId(mVUlow.VF_write.reg, mVUlow.VF_write.reg, 0xf, 0);
		mVUCompileXorQ_emit_oaknut(t2, t2, t3);
		mVUCompileXorQ_emit_oaknut(t3, t3, t2);
		mVUCompileXorQ_emit_oaknut(t2, t2, t3);
		...
		const int t4 = mVU.regAlloc->allocRegId(-1, mVUlow.VF_write.reg, 0xf);
		mVUCompileMoveQ_emit_oaknut(t4, t2);
		mVU.regAlloc->clearNeededXmmId(t4);
...
__fi void handleBadOp(mV, int count)
{
	...
	if (mVUinfo.isBadOp)
	{
		...
		mVUCompileBadOpCall_emit_oaknut(mVU);
...
void mVUtestCycles(microVU& mVU, microFlagCycles& mFC)
{
	...
	if (EmuConfig.Gamefixes.VUSyncHack) {
		mVUCompileCompareCycles_emit_oaknut(mVU, mVUcycles);
    }
	else {
		mVUCompileCompareCycles_emit_oaknut(mVU, 1);
    }
...
static __fi void mVUCompileCompareCycles_emit_oaknut(mV, u32 cycles)
{
	...
	oakAsm->SUBS(OAK_WSCRATCH, OAK_WSCRATCH, cycles);
	recEndOaknutEmit();
}
...
{
	recBeginOaknutEmit();
	oakAsm->MOV(OAK_WSCRATCH, cycles);
	oakStore32(OAK_WSCRATCH,
...
static void mvuPreloadRegisters(microVU& mVU, u32 endCount)
{
	...
	auto preloadVF = [&mVU, &vfs_loaded, &free_regs](u8 reg)
	{
		...
		const int vfReg = mVU.regAlloc->allocRegId(reg);
		mVU.regAlloc->clearNeededXmmId(vfReg);
		vfs_loaded |= (1u << reg);
		free_regs--;
	};
	...
	auto preloadVI = [&mVU, &vis_loaded, &free_gprs](u8 reg)
	{
		...
		mVU.regAlloc->clearNeeded(mVU.regAlloc->allocGPRId(reg));
		vis_loaded |= (1u << reg);
		free_gprs--;
	};
...
void* mVUcompile(microVU& mVU, u32 startPC, uptr pState)
{
	...
	u8* thisPtr;
	bool startedOakBlock = false;
	if (oakHasBlock())
	{
		thisPtr = oakGetCurrentCodePointer();
	}
	else
	{
	...
	for (; x < endCount; ++x)
	{
		...
		if (mVUup.mBit)
		{
			mVUBranchOrVuFlags_emit_oaknut(mVU, VUFLAG_MFLAGSET);
		}

		if (isVU1 && mVUlow.kickcycles && CHECK_XGKICKHACK)
		{
			mVU_XGKICK_SYNC_oaknut(mVU, false);
		}
		...
		if (!mVUinfo.isBdelay && !mVUlow.branch) //T/D Bit on branch is handled after the branch, branch delay slots are executed.
		{
			...
			else if (mVUup.mBit && !mVUup.eBit && !mVUinfo.isEOB)
			{
				...
				mVUCompileSaveLpState_emit_oaknut(mVU, cpS);
...
//					condBranch(mVU, mFC, Jcc_Equal);
                    condBranch(mVU, mFC, oak::Cond::EQ);
					goto perf_and_return;
				case 4: // IBGEZ
//					condBranch(mVU, mFC, Jcc_GreaterOrEqual);
                    condBranch(mVU, mFC, oak::Cond::GE);
					goto perf_and_return;
				case 5: // IBGTZ
//					condBranch(mVU, mFC, Jcc_Greater);
                    condBranch(mVU, mFC, oak::Cond::GT);
					goto perf_and_return;
				case 6: // IBLEQ
//					condBranch(mVU, mFC, Jcc_LessOrEqual);
                    condBranch(mVU, mFC, oak::Cond::LE);
					goto perf_and_return;
				case 7: // IBLTZ
//					condBranch(mVU, mFC, Jcc_Less);
                    condBranch(mVU, mFC, oak::Cond::LT);
					goto perf_and_return;
				case 8: // IBNEQ
//					condBranch(mVU, mFC, Jcc_NotEqual);
                    condBranch(mVU, mFC, oak::Cond::NE);
					goto perf_and_return;
			}
		}
	}
