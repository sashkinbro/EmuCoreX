...
static void ata_build_identify()
{
	...
	if (ACATA::TH::IMAGE || ACATA::TH::isCHD) {
...
void ACATA::handle_cmd(u16 val) {
    ...
    case ATA_C_READ_SECTOR: {
		...
		if ((!ACATA::TH::IMAGE && !ACATA::TH::isCHD) || total > sizeof(ata_pio_buf)) {
		...
		if (ACATA::TH::isCHD)
		{
			ok = CHD.ReadLogicalSectors(lba, count, ATA_SECTORSIZE, ata_pio_buf);
		}
		else
		{
			FileSystem::FSeek64(ACATA::TH::IMAGE, (s64)lba * ATA_SECTORSIZE, SEEK_SET);
			ok = (fread(ata_pio_buf, 1, total, ACATA::TH::IMAGE) == total);
		}
		if (!ok) {
			Console.Error("ATA_C_READ_SECTOR: read failed (%u bytes) at LBA %u", total, lba);
			R_STATUS |= ATA_STAT_ERR;
            R_ERROR = ATA_ERR_ABORT;
            ACCORE::intr(ACCORE::INTRN_ATA);
            break;
...
void ACATA::Reset()
{
	...
	R_STATUS_ALT = 0;
	R_CONTROL = 0;
	MediaType = ACMEDIATYPE::ACUNK;
	imgpath.clear();
	TH::IMAGESIZE = 0;
	TH::nsector = 0;
	TH::LBA = 0;
	TH::sectorsize = ACATAPI::CONSTANTS::DVD_SECTORSIZE;
}
...
void ACATA::SetEnv(std::string ata_img_path, std::string ata_img_filename, std::string Media) {
    ...
    if (Path::IsAbsolute(ata_img_filename) ||
		StringUtil::StartsWithNoCase(ata_img_filename, "content://"))
	...
	ACATA::TH::sectorsize = (ACATA::MediaType == ACMEDIATYPE::ACHDD) ?
		512 : ACATAPI::CONSTANTS::DVD_SECTORSIZE;
