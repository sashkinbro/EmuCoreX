...
std::FILE* FileSystem::OpenCFile(const char* filename, const char* mode, Error* error)
{
	...
	if (file_name.rfind("content://", 0) == 0)
		fp = fdopen(FileSystem::OpenFDFileContent(file_name.c_str()), "rb");
	else
		fp = std::fopen(file_name.c_str(), mode);
	if (!fp)
...
int FileSystem::OpenFDFile(const char* filename, int flags, int mode, Error* error)
{
	...
	const std::string file_name(filename);
	if (file_name.rfind("content://", 0) == 0)
		fd = FileSystem::OpenFDFileContent(file_name.c_str());
	else
...
bool FileSystem::StatFile(const char* path, FILESYSTEM_STAT_DATA* sd)
{
	...
	{
		ManagedCFilePtr fp = OpenManagedCFile(path, "rb");
		if (!fp)
			return false;

		return StatFile(fp.get(), sd);
	}
