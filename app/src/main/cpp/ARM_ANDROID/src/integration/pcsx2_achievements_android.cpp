...
namespace Achievements
{
	...
	[[maybe_unused]] static constexpr float LEADERBOARD_STARTED_NOTIFICATION_TIME = 3.0f;
	[[maybe_unused]] static constexpr float LEADERBOARD_FAILED_NOTIFICATION_TIME = 3.0f;
...
{
	*http = HTTPDownloader::Create(
#if defined(__ANDROID__)
		emucorex::android::GetRetroAchievementsUserAgent()
#else
		Host::GetHTTPUserAgent()
#endif
	);
	if (!*http)
	{
	...
	if (list)
	{
		for (u32 bucket_index = 0; bucket_index < list->num_buckets; bucket_index++)
		{
			...
			for (u32 achievement_index = 0; achievement_index < bucket.num_achievements; achievement_index++)
			{
				...
				out.append(",\"state\":");
				out.append(std::to_string(achievement->state));
				out.append(",\"bucket\":");
				out.append(std::to_string(achievement->bucket));
				out.append(",\"measuredProgress\":");
				AppendJSONString(out, achievement->measured_progress);
				out.append(",\"measuredPercent\":");
				out.append(fmt::format("{}", achievement->measured_percent));
				out.append(",\"rarity\":");
