...
object BiosValidator {
    ...
    private val validExtensions = setOf("bin", "rom", "mec", "nvm", "elf")
    ...
    fun hasUsableBiosFiles(context: Context, rawPath: String?): Boolean {
        ...
        return if (rawPath.startsWith("content://")) {
            ...
            dir.isDirectory && dir.walkTopDown().maxDepth(2).any(::isLikelyBiosFile)
        }
    }
    ...
    private fun containsBiosFile(root: DocumentFile): Boolean {
        for (child in root.listFiles()) {
            ...
            if (child.isFile && isLikelyBiosName(child.name)) return true
        }
        return false
    }

    private fun isLikelyBiosFile(file: File): Boolean {
        return file.isFile && isLikelyBiosName(file.name)
    }
    ...
    fun isLikelyBiosName(name: String?): Boolean {
        ...
        return ext in validExtensions && (fileNameHints.any(fileName::contains) || extraArtifactHints.any(fileName::contains))
    }
}
